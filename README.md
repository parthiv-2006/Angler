...
Everything on screen is real, and both paths are live. The four seeded verticals and their bundled sample ad sets load instantly from cache with zero credentials; type anything else into the box and Angler runs the pipeline for real — a live Meta Ad Library pull, live Claude analysis, the full report. Live usage is protected by per-IP rate limits and a global daily budget, and it always degrades to a friendly pointer back at the seeded verticals rather than an error. See [Data provenance](#data-provenance-no-fabricated-data).
...
**1. Stream the live run.** The live path is on: any vertical typed into the box triggers a real scrape and a real model pipeline, bounded by a global daily budget (`LIVE_DAILY_CAP`). The long steps — a 45-second Ad Library pull, a multi-minute brief batch — currently run as single requests with a status log. Next: stream partial results (ads as they land, briefs as they're written) so the live experience feels as immediate as the seeded one.
...
- **Live pulls are real pulls.** Typing a novel vertical runs the public Apify actor against the Meta Ad Library and analyzes the results with Claude on the spot; nothing is staged. Results are write-through cached in Supabase, so repeat searches of the same vertical are instant and free. Live spend is bounded by per-IP rate limiting plus a global daily request budget, and any live failure degrades to a friendly pointer at the seeded verticals instead of an error screen.
...
**AI cost:** seed verticals are pre-analyzed and served from cache, so clicking through the demo makes zero API calls. A full live run on a novel vertical (15 ads through DNA extraction, clustering, and 12 generated briefs) costs on the order of $0.10 at Anthropic's Sonnet pricing, plus one Apify actor run for the scrape. A global daily cap (`LIVE_DAILY_CAP`, default 150 metered requests) bounds worst-case spend; past it, live requests get a friendly "budget used up" message while seeded verticals keep working. Gemini Flash is a one-env-var fallback at effectively $0 on the free tier.
